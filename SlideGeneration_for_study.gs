...
function generateStructuredOutput(article) {
  ...
  const messages = [
    {
      role: "system",
      content: "You are an expert at structured data extraction of epidemiology. You will be given unstructured text from a research paper of emergency infections and should convert it into the given structure."
    },
    {
      role: "user",
...
function createGoogleSlide(structuredData, article) {
  ...
  const fileName = `${fileNamePubDate}_${article.journal.replace(/[^\w\s]/g, '')}_${article.pubmedId}`;
  ...
  try {
    ...
    // 2枚目のスライドがない場合は1枚目を複製
    if (slides.length < 2) {
      firstSlide.duplicate();
    }
    const secondSlide = presentation.getSlides()[1];
    
    // 1枚目のスライド用のプレースホルダー置換データ
    const firstSlideReplacements = {
      '{Title}': article.title || '不明',
      '{PublicationDate}': displayPubDate || '不明',
      '{Citation}': article.citation || '不明',
      '{PMID}': article.pubmedId || '不明',
      '{abstract}': article.abstract || '不明',
      '{Journal}': article.journal || '不明'
    };
    
    // 2枚目のスライド用のプレースホルダー置換データ
    const secondSlideReplacements = {
    ...
    // 1枚目のスライドのプレースホルダーを置換
    Object.entries(firstSlideReplacements).forEach(([placeholder, value]) => {
      firstSlide.replaceAllText(placeholder, value);
    });
    
    // 2枚目のスライドのプレースホルダーを置換
    Object.entries(secondSlideReplacements).forEach(([placeholder, value]) => {
      secondSlide.replaceAllText(placeholder, value);
    });
